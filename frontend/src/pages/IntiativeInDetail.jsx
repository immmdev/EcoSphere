...
function InitiativeInDetail() {
    ...
    // setting initiative
    const [initiative, setInitiative] = useState(location.state || null);
    ...
    useEffect(() => { setInitiative(location.state || null) }, [location.state])
    ...
    //  Fetch joins & full initiative details (also covers direct navigation with no location.state)
    const fetchJoins = () => {
        initiativeService
            .fetchJoins(id)
            .then((res) => {
                ...
                if (res.data.initiative) {
                    setInitiative(res.data.initiative);
                }
            })
            .catch((err) => {
                console.log(err);
    ...
    // fetch joins on first mount / when the id changes
    useEffect(() => { fetchJoins(); }, [id]);
    ...
    const leaderName = initiative.leader?.name;
    const isMember = userId && members.includes(userId);
                ...
                <div className="bg-green-50 shadow-lg rounded-lg overflow-hidden">
                    ...
                    <div className="p-6">
                        ...
                        <div className="flex justify-between text-sm text-green-900 mb-4">
                            <span>
                                <b>LEAD {leaderName ? leaderName.toUpperCase() : "Unknown"}</b>
                            </span>
                        ...
                        <div className='flex gap-4 my-2 hover:cursor-pointer'>
                            ...
                            {/* conditional rendering of join button */}
                            {!token ? (
                                <span className="text-green-900 text-sm self-center">Log in to join this initiative ({joins})</span>
                            ) : isMember ? (
                                <button
                                    disabled={loading}
                                    onClick={() => {
